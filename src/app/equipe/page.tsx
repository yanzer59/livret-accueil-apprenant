...
function DField({ l1, n1, l2, n2, data, update, req1, req2 }: { l1: string; n1: string; l2: string; n2: string; data: Record<string, unknown>; update: (n: string, v: string) => void; req1?: boolean; req2?: boolean }) {
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
      <Field label={l1} name={n1} data={data} update={update} required={req1} />
      <Field label={l2} name={n2} data={data} update={update} required={req2} />
    </div>
  );
}
  ...
  return (
    <PageSection num={16} title="Equipe pedagogique et contacts">
      ...
      <SaveBar requiredFields={[
        { name: "equipe_dir_nom", label: "Nom direction" }, { name: "equipe_dir_prenom", label: "Prenom direction" },
        { name: "equipe_dir_tel", label: "Tel. direction" }, { name: "equipe_dir_email", label: "E-mail direction" },
        { name: "equipe_resp_nom", label: "Nom resp. peda" }, { name: "equipe_resp_prenom", label: "Prenom resp. peda" },
        { name: "equipe_resp_tel", label: "Tel. resp. peda" }, { name: "equipe_resp_email", label: "E-mail resp. peda" },
        { name: "equipe_formateur_nom", label: "Nom formateur" }, { name: "equipe_formateur_prenom", label: "Prenom formateur" },
        { name: "equipe_formateur_tel", label: "Tel. formateur" }, { name: "equipe_formateur_email", label: "E-mail formateur" },
        { name: "equipe_tel_standard", label: "Tel. standard" }, { name: "equipe_email_general", label: "E-mail general" },
        { name: "equipe_adresse", label: "Adresse CFA" },
      ]} />
    </PageSection>
  );
}
